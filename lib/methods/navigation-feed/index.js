...
const handleMultipleRequest = require('./handle-multiple-requests');
...
function navigationFeed(query = {}, options, callback) {
  ...
  const requestOptions = _.merge(omitDefaultRequestParams(options), {
    ...
    uri: `basic/api/wiki/${wikiLabel}/nav/feed`,
    json: true,
  });
  ...
  httpClient.makeRequest(requestOptions, (requestError, response, body = {}) => {
    ...
    const { fromCache, statusCode } = response;
    const { isStubTypeAllowed } = requestOptions;
    ...
    // if stubType is allowed by the request options or if we do not have stubType in our response data,
    // than cache the data if it doesn't come from the cache already
    ...
    if (!fromCache) {
      handleMultipleRequest({ httpClient, requestOptions, response, body, callback });
      return;
    }
    ...
    // since this version contains stubType data, which are not allowed.
    // we re-initiate the same request while skipping the "cache" request phase list handler
    // which resolves to invoking ".handleMultipleRequest()" method
