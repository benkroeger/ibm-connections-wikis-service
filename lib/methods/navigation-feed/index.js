...
/**
 * Usually ibm connections origin server sends next response header:
 * 'cache-control': 'private, no-cache, no-store, max-age=0, ...'
 * These options actually notify the caching plugin to do next:
 * 1. Store when flag 'private' is present (happens when 'user' object is present as well)
 * 2. Store when flag 'no-store' is present in the response header
 * 3. Ignore validators when flag 'max-age=0' and use ttl
 */
const defaultNavigationFeedRequestOptions = {
  plugins: {
    cacheRedis: {
      ttl: 60 * 60 * 12,
      delayCaching: true,
...
function navigationFeed(query = {}, options, callback) {
  ...
  const requestOptions = _.merge(
    defaultNavigationFeedRequestOptions,
  ...
  httpClient.makeRequest(requestOptions, (requestError, response, body = {}) => {
    ...
    const { isStubTypeAllowed, plugins: { cacheRedis: { delayCaching } } } = requestOptions;
    ...
    if (isStubTypeAllowed || !hasStubType) {
      ...
      // otherwise return the cached response body
      callback(null, { items: body });
      return;
    }
